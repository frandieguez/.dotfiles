...
function! taskpaper#put(...)
    ...
    if indent > 0
        let project_depth = len(matchstr(getline('.'), '^\s*'))
        echo indent
...
function! taskpaper#newline()
    ...
    \  synIDattr(synID(lnum - 1, 1, 1), "name") != 'taskpaperListItem' )
        return ''
    endif

    let pline = getline(lnum - 1)
    let depth = len(matchstr(pline, '^\s*')) + 1

    if synIDattr(synID(lnum - 1, 1, 1), "name") == 'taskpaperListItem'
        let depth = len(matchstr(pline, '^\s*'))
    endif
